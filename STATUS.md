...
All five labeling backends (`DBSCANConfig`, `HDBSCANConfig`, `HierarchicalConfig`, `VoronoiConfig`, `MRFDensityClusterConfig`) and both spatial-statistic backends (`HopkinsConfig`, `VoronoiDensityConfig`) are live end-to-end with full documentation, README, and api_overview. `cluster(smld, cfg)` is non-mutating (KB V9 deepcopy); `cluster_statistics(smld, cfg)` is pass-through (KB V10 same-reference). Test suite split into a fast tier (161/161 in ~30 s, default `Pkg.test()`) and a thorough tier gated by `SMLM_TEST_FULL=true|1|yes` (981/981 in ~35 s) — cross-package convention shared with SMLMAnalysis / SMLMBaGoL / SMLMDriftCorrection. **Diagnosis loop now active.** A synthetic A431-mimic dataset (`dev/scripts/output/synthetic_smld.jld2`, 13,579 emitters: 7,968 low-density / 5,611 high-density across 12 patches in a 5×5 μm ROI with right-edge bite) is the working bench for MRF improvement. MRF currently lands at 79.85% headline accuracy on this dataset — best of all backends (DBSCAN, HDBSCAN, voronoi-GMM trail) but with a concentrated false-negative band at patch interiors (visible in `dev/scripts/output/mrf_eval.png`). The next 4 HIGH priorities target closing the GT gap. Round dispatch migrated to fork-based architecture 2026-04-27 (no tmux launcher, no lock file). Pending external action: Priority 5 — Keith creating `JuliaSMLM/SMLMClustering.jl` so @analysis can `[sources]`-wire.
...
1. [HIGH] Diagnose MRF interior false-negative band on the synthetic A431-mimic. Visual inspection of `dev/scripts/output/mrf_eval.png` shows cyan FN clustering at high-density patch interiors (especially elongated fibers AR 5-20), while patch edges classify well. Hypothesis: Voronoi cell area shrinks within tightly-packed cores in a way that pushes them back into the low-regime tail of the GMM, OR the Potts smoothness is over-smoothing across regime boundaries from sparse-edge contamination. The synthetic is good (Keith 2026-04-27); we should be able to recover near-GT match.
   - add: `dev/scripts/diagnose_mrf_interior.jl` — for each labeled patch on `synthetic_smld.jld2`, compute (a) per-emitter Voronoi area distribution, (b) per-emitter ICM regime vs ground truth, (c) per-iteration ICM flip count, and write per-patch diagnostic figure showing area histogram, regime overlay, and FN spatial concentration. Iterate at least one MRF code change suggested by the diagnostic — candidates include kNN density estimator (P17 part c brought forward), per-emitter unary weighting, or anisotropic smoothness — into `src/backends/mrf_density.jl` behind a config switch.
   - test: Round must Read `dev/scripts/output/mrf_interior_diagnosis.png` (per phase-3-extras visual-inspection rule) and verify the FN-band hypothesis. Headline accuracy on the synthetic must clear 85% with the chosen mitigation, AND the per-patch FN-rate at patch interiors (within 100 nm of patch centroid) must drop measurably vs the v1 79.85% baseline. Add a regression test in the thorough tier that loads the synthetic and asserts the new accuracy floor.
   - doc: New KB entry (V12 or D2) — cause + chosen mitigation, OR confirmation that the FN band is irreducible at this density ratio. Update `src/backends/mrf_density.jl` docstring if a code change lands. — TODO
2. [HIGH] Density-ratio operational-floor sweep. The synthetic uses a 2× density ratio; real dSTORM ratios may run 1.2-3×. MRF's lower bound is unknown — we need the curve before deploying on genmab paper data.
   - add: `dev/scripts/density_ratio_sweep.jl` regenerates the synthetic at ratios {1.2×, 1.5×, 2.0×, 3.0×, 5.0×}, runs MRF + DBSCAN + HDBSCAN + voronoi-GMM on each, writes `dev/scripts/output/density_ratio_sweep.csv` (per-ratio accuracy/precision/recall) + `dev/scripts/output/density_ratio_sweep.png` (curve plot for all 4 backends).
   - test: Round must Read the PNG and confirm (a) MRF curve is monotonic-ish with ratio (non-monotonic = anomaly worth flagging), (b) MRF beats baselines across the sweep, (c) operational floor identified (min ratio at which MRF accuracy > 75%). Per phase-3-extras visual-inspection rule.
   - doc: KB V13 records operational floor + curve shape. README's MRF section gains a "When MRF works" subsection citing the floor. — TODO
...
6. [LOW] MRF v2 enhancements (held for after the diagnosis loop closes): (a) graph-cuts MAP inference (binary or α-expansion) replacing ICM — `inference=:graph_cuts` symbol slot reserved but raises `ArgumentError`; needs new dep (BoykovKolmogorov.jl, GraphsFlows.jl) or in-house max-flow. (b) Soft-posterior output mode — `extras[:posterior_per_emitter]::Matrix{Float64}` (n × n_regimes) when a flag is set. (c) k-NN density estimator alternative to Voronoi — `density_estimator=:knn` with `density_k`; if Priority 1 brings (c) forward, this entry shrinks to (a)+(b). All additive — current pipeline stays default.
   - add: TBD per (a)/(b)/(c) selection
   - test: TBD
   - doc: TBD — TODO

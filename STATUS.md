...
All three labeling backends (`DBSCANConfig`, `HierarchicalConfig`, `VoronoiConfig`) and both spatial-statistic backends (`HopkinsConfig`, `VoronoiDensityConfig`) are live end-to-end with full documentation. `cluster(smld, cfg)` is non-mutating (deep-copies emitters per KB V9); `cluster_statistics(smld, cfg)` is pass-through (returns input SMLD reference per KB V10). Task A (P11+P12+P13+P14) and Task B (P15) both landed via direct execution on 2026-04-27, bypassing the round dispatcher for the paper-genmab-hexabody v1 hour-scale push. **Test suite now split into a fast tier (101/101, default `Pkg.test()`) and a thorough tier gated by `SMLM_TEST_FULL=true|1|yes` (861/861); shared cross-package convention with SMLMAnalysis / SMLMBaGoL / SMLMDriftCorrection.** README and api_overview cover both entry points, the labeling-vs-diagnostics split, and the summary-scalar+extras convention. Only remaining open work is Priority 7 (Keith creating the `JuliaSMLM/SMLMClustering.jl` GitHub org repo so @analysis can wire in via `[sources]`).

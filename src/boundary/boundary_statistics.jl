...
These duplicate fields already available on `ClusterInfo` (the secondary
output of `cluster()`), unlike `n_clusters` which is intentionally *not*
duplicated — included here so a `ClusterBoundaryStatisticsInfo` is a
self-contained report.
- `n_locs_in::Int`, `n_clustered::Int`, `n_noise::Int`: total/clustered/noise
  emitter counts in `smld_out`.
- `algorithm::Symbol`: the clustering backend that produced `smld_out`'s
  labels, passed in via the `algorithm` keyword (`:unknown` if omitted).
- `shrink::Float64`: the alpha-shape tightness used for `binfo`'s boundary
  (same as `binfo.shrink`).
...
"""
    boundary_statistics(smld_out, binfo::ClusterBoundaryInfo; per_dataset=false)
...
- `per_dataset`: must match the value used in the `BoundaryClustersConfig`
  ...
  built.
- `algorithm`: the clustering backend identifier to record on the returned
  `info` (typically `info_cluster.algorithm` from the preceding `cluster()`
  call); defaults to `:unknown` if omitted.
...
function boundary_statistics(smld_out::SMLMData.BasicSMLD,
    ...
    n_locs_in   = length(smld_out.emitters)
    n_clustered = count(e -> e.id > 0, smld_out.emitters)
    n_noise     = count(e -> e.id == 0, smld_out.emitters)

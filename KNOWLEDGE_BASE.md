...
### V15 — Soft calibrated Gaussian unaries make the MRF behave like the intended spatial hidden-state model

**Decision:** `MRFDensityClusterConfig` now accepts `regime_gaussians::Union{Nothing, NamedTuple{(:means, :vars, :weights)}} = nothing`. When supplied, the backend bypasses both per-ROI GMM EM and hard `regime_thresholds`, and instead builds the same soft Gaussian unary matrix as auto-mode: `U[i, k] = -log(w_k * N(log_rho[i] | μ_k, σ_k²))`. This is the right calibration workflow when a trusted calibration ROI exists: emissions are fixed and reproducible, but borderline emitters can still be rescued by high-density neighbors through the Potts pairwise term.

`calibrate_regime_gaussians(smld; n_regimes=2, density_estimator=:knn, density_k=20)` fits the calibrated `(means, vars, weights)` tuple in log-density space. `calibrate_regime_thresholds` is now a thin wrapper around this helper followed by analytic Bayes boundary extraction. `regime_thresholds` remains supported as conservative hard binning, but it is not the preferred path for the "missing high-density interiors" failure mode because its `1e6` penalty blocks neighbor rescue.

**Evidence:** Direct implementation 2026-04-30. Fast regression constructs three neighboring nodes where one observation is just below the hard threshold: hard-threshold unaries leave labels `[1, 2, 2]`, while soft Gaussian unaries with the same Potts weight converge to `[2, 2, 2]`. Held-out A431-mimic check using calibration seed 20260429 and query seed 20260430: at 2× density ratio, soft calibrated mode reaches 92.66% accuracy versus GMM auto 93.72% (Δ = −1.06 pp, within the original ±2 pp gate) and hard-threshold override 88.25%; at 1.5×, soft calibrated mode reaches 76.95% versus hard threshold 75.90% and GMM auto 72.10%. Test suite green: fast 185/185, thorough 1025/1025.

**Scope:** Applies to `MRFDensityClusterConfig` with Gaussian-like log-density regimes, especially the 2-regime kNN-density workflow on patch/background SMLM data. Soft calibrated emissions do not solve all MRF limitations: ICM is still local MAP inference, and low-contrast data without a representative calibration ROI can still be fragile. Graph-cuts/belief-propagation style inference remains a separate v2 enhancement, but soft calibrated unaries fix the immediate "2D HMM should rescue interior misses but hard thresholds prevent it" issue without adding dependencies.

### V16 — kNN-MRF patch-size operating range at 2× contrast: ≥0.5 μm clears 75% gate, ≥1.0 μm clears 85% gate; collapses below the kNN-ball scale

**Decision:** On the 5×5 μm A431-mimic with fixed 2× density ratio (RHO_LOW=500/μm², RHO_HIGH_BONUS=500), the `MRFDensityClusterConfig` kNN-density pipeline (`density_estimator=:knn, density_k=20, n_regimes=2`) has a sharply patch-size-dependent operating regime:

- patch size ≥ 1.0 μm — kNN-MRF holds **88-89% accuracy** (88.72 / 89.11 / 88.31% at nominal 1.0 / 2.0 / 3.0 μm). 85% gate cleared.
- patch size ≈ 0.5 μm — kNN-MRF clears the **75% gate** at 79.25% accuracy. 85% gate not yet reached.
- patch size ≤ 0.3 μm — kNN-MRF **collapses to 62.7%** (FP-saturation regime: when the kNN(k=20) ball radius ~√(k / π·ρ_high) ≈ 80 nm becomes comparable to the structure half-width, the ball spills into background and the GMM regime split flips foreground/background — same mechanism as V12's regime bound, here triggered by patch geometry rather than density). voronoi-GMM (per-emitter independent) actually beats kNN-MRF at this scale (65.3% vs 62.7%) because the smoothness term that helps elsewhere amplifies the kNN degeneracy here.

voronoi-GMM is a strict-loss baseline at 2× contrast across the entire patch-size sweep (~65-72%, never crosses the 75% gate) — at this contrast it is bounded by per-emitter log-ρ separability and patch size barely affects it. Its small-patch-end advantage at 0.3 μm is below the gate and not operationally useful; at 2× ratio kNN-MRF is the right backend whenever patch size ≥ 0.5 μm.

**Operational guidance.** For 2× density ratio data:

- patch size ≥ 1.0 μm — deploy default kNN-MRF (k=20). Expect 88-89% accuracy on A431-mimic-like geometry.
- 0.5 μm ≤ patch size < 1.0 μm — kNN-MRF still beats voronoi-GMM but is below the 85% gate; consider lowering `density_k` (V12's k-vs-structure-half-width bound suggests dropping to k=8-10 narrows the ball below the patch half-width).
- patch size < 0.5 μm — kNN-MRF default (k=20) is below the 75% gate. Either drop `density_k` aggressively to keep the ball inside the structure, or fall back to voronoi-GMM as a graceful-loss backend (P8 k-sensitivity sweep is the natural follow-up that would map this regime).

The 1.0 μm point reproduces Round 012's 89.11% headline (same RNG seed, same backend, same patch geometry) — the `patch_scale=1.0` refactor of `simulate_a431_mimic.jl` is semantics-preserving. This serves as a positive control on the sweep refactor.

**Evidence:** Round 015 sweep at `patch_scale ∈ {0.15, 0.25, 0.5, 1.0, 1.5}` (nominal mean rect length `{0.3, 0.5, 1.0, 2.0, 3.0} μm`). Script `dev/scripts/patch_size_sweep.jl`, results CSV at `dev/scripts/output/patch_size_sweep.csv`, aggregate accuracy curve at `dev/scripts/output/patch_size_sweep.png` (log-x patch size vs accuracy, both backends, dashed 75% + dotted 85% gates), 5-panel SMLMRender categorical TP/TN/FP/FN circle plot at `dev/scripts/output/round_015_smlmrender_categorical.png` (kNN-MRF predictions only). Same RNG seed across the sweep so RNG draws are stable; only `patch_scale` multiplies linear dimensions of rect (L ∈ [s, 3s], b ∈ [0.5s, s]) and ellipse (a ∈ [0.5s, s], b ∈ [0.25s, 0.5s]). Visual diagnostic on the 0.3 μm panel shows the FP-saturation signature (V13 mechanism, here triggered by ball spillage rather than low contrast) — green-FP across the ROI mixed with red-FN and orange-TP; from 0.5 μm onward TP-orange patch interiors emerge cleanly.

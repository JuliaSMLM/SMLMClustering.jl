...
**Scope:** Applies to all current and future `cluster(smld, cfg::AbstractClusterConfig)` methods. A hypothetical `cluster!` mutating variant would be a separate method with the `!` suffix; the un-suffixed entry point must never mutate. Note this contrasts with the forthcoming `cluster_statistics(smld, cfg)` entry point (P11), which is a passthrough (SMLD reference is the same as input) because it writes nothing — that asymmetry is intentional and must be documented on both docstrings.
